...
import { Search, Filter, Package, ChevronRight, Calendar, Clock, ArrowDownUp, X, CheckCheck, Truck, ShoppingBag, RefreshCw } from 'lucide-react';
...
const Orders = () => {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'Processing': return <ShoppingBag className="w-4 h-4 text-yellow-500" />;
      case 'Cancelled': return <X className="w-4 h-4 text-red-500" />;
      case 'Returned': return <RefreshCw className="w-4 h-4 text-purple-500" />;
      ...
      <div className="sticky top-0 bg-white dark:bg-black z-10 pb-2">
        <div className="flex justify-between items-center mb-3 border-b pb-2">
          <div>
            ...
            <p className="text-xs text-gray-500">{filteredOrders.length} orders from all time</p>
          </div>
            ...
            <div key={idx} className="border rounded-lg shadow-sm p-3 hover:shadow-md transition">
              <div className="flex justify-between items-start mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`${getStatusColor(order.statusColor)} px-2 py-0.5 rounded-full text-xs font-medium`}>
                    {order.status}
                  </span>
                </div>
                <div className="text-right">
                  <span className="text-xs text-gray-700 font-medium">{order.id}</span>
              ...
              <div className="flex items-center justify-between border rounded-md p-2 ">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">Brand: {order.brand}</p>
              ...
              <div className="mt-2 p-2 rounded-md">
                ...
                {order.estimatedShipping && (
                  <p className="text-xs text-blue-600">Estimated shipping: {order.estimatedShipping}</p>
                )}
              </div>
                  ...
                  {order.status === 'Delivered' && (
                    <div className="flex gap-1">
                      {[...Array(5)].map((_, i) => (
                        <svg 
                          key={i} 
                          className={`w-4 h-4 ${i < (order.rating || 0) ? 'text-yellow-500' : 'text-gray-300'}`} 
                          fill="currentColor" 
                          viewBox="0 0 24 24"
                        >
